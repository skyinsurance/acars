...
class SharefileComponent extends Component {
	...
	/*public function create_employee($hostname, $client_api_id, $client_secret, $username, $password, $client_id, $client_logged_id, $email, $firstname, $lastname) {
		...
		$clientpassword = \Yii::$app->EncryptDecrypt->encrytedUser($clientpassword);
		//'Password1!';
		...
		$client = array("Email"=>$email, "FirstName"=>$firstname, "LastName"=>$lastname,
				"Password"=>$clientpassword,"IsEmployee"=>TRUE,"IsAdministrator"=>TRUE,"CanCreateFolders"=>TRUE,"CanUseFileBox"=>TRUE,
				"CanManageUsers"=>TRUE,
	...
	public function create_company_folder($token, $parent_id, $name, $client_logged_id, $client_id, $company_id){
		...
		if ($http_code == 200) {
			...
			if($model_sharefile->save())
			{
				$success_result = array("result"=>"success", "id"=>$item->Id);
				return 	json_encode($success_result);
				//return 'success';
			}
			else{
				//return 'error occured';
				$success_result = array("result"=>"error occured");
				return 	json_encode($success_result);
			}
	...
	public function create_extra_sharefile_folder($hostname, $client_api_id, $client_secret, $client_logged_id, $client_id, $new_username, $new_password, $name, $company_id){
		...
		$response = json_decode($result);
		if($response->result = 'success'){
			$success_result = array("result"=>"success", "id"=>$response->id);
			return 	json_encode($success_result);
			//return 'success';
		}
		else{
	...
	public function create_client($hostname, $client_api_id, $client_secret, $username, $password, $email, $firstname, $lastname, $client_id, $client_logged_id){
		...
		//echo "POST ".$uri."\n";
		/*$email = 'banelasainath@gmail.com';
		$firstname = 'Sainath';
		$lastname = 'Banela';*/
		
		$client = array("Email"=>$email, "FirstName"=>$firstname, "LastName"=>$lastname,
				"Password"=>$clientpassword,"addPersonal"=>TRUE,"notify"=>FALSE,
				"Preferences"=>array("CanResetPassword"=>FALSE, "CanViewMySettings"=>FALSE));
		$data = json_encode($client);
		...
		if ($http_code == 200) {
			...
			if($model_sharefile->save())
			{
				...
				//return 	$result;
				$success_result = array("result"=>"success", "id"=>$sharefile_client_id);
				return 	json_encode($success_result);
			}
			else{
				$success_result = array("result"=>"error occured");
