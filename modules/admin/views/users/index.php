	 ...
	 <?php ActiveForm::end(); ?>
        ...
        'columns' => [
			...
			'value' => function ($model, $key, $index, $widget) {
				...
				{
					$permissions2 = 'No Permissions assigned';
				}
				return $permissions2;
			},
			'headerOptions' => [ 
...
if($logged_user_id != $user_id){
	...
	case '1': $status_label = '<a class="pointer" id="'.$user_id.'" onclick="activateUser('.$user_id.',1);">Active</a>';
	break;
	case '0': $status_label = '<a class="pointer" style="color:red;" onclick="activateUser('.$user_id.',0);">Inactive</a>';
	break;
...
'buttons' => [
    'my_button' => function ($url, $model, $key) {
		...
		$encrypt_component = new EncryptDecryptComponent();
		$user_id = $encrypt_component->encrytedUser($user_id);
        return Html::a('<span class="fa fa-edit"></span>',['/admin/users/updateusers', 'id' => $user_id], ['class' => 'pointer']) ;
    },
]
			],
        ],
