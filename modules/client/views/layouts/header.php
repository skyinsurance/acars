...
<header class="main-header">
    	...
    	<a class="logo" href="<?php echo Yii::$app->getUrlManager()->getBaseUrl(); ?>/client/companies" style="padding: 0 2px;"><div class="col-md-2 padding-0"><img src="<?php echo Yii::$app->getUrlManager()->getBaseUrl(); ?>/Images/profile_image/brand_logo/<?php echo $brand_logo;?>" class="" height="40px;"></div></a>
    ...
    <nav class="navbar navbar-static-top" role="navigation" >
        ...
        $model_companies = new TblAcaCompanies();
         $all_companies = TblAcaCompanies::FindallwherecompanyIds($company_ids);
         ?>
...
$user_id = $encrypt_component->encrytedUser($companies->company_id);
								?>
							<option value="<?php echo $user_id; ?>" <?php if($user_id == $encrypt_company_id){?> selected <?php }?>><?php echo $companies->company_name; ?> (<?php echo $companies->company_client_number; ?>) - <?php echo $companies->tbl_aca_company_reporting_period->year->lookup_value;?></option>
<?php } } } ?>
         </select>
        ...
        <div class="navbar-custom-menu pull-right col-xs-2 col-lg-3 col-md-2">
            ...
            <li class="dropdown user user-menu" >
					...
					<?php if(!empty(Yii::$app->Permission->Profileimage($logged_id))){?>
                        <img src="<?php echo Yii::$app->getUrlManager()->getBaseUrl(); ?>/Images/profile_image/<?php echo Yii::$app->Permission->Profileimage($logged_id);?>" class="user-image" alt="User Image"/>
						<?php } ?>
                        <span style="color:white!important;" class="hidden-xs"><?php  $string = (strlen($name) > 10) ? substr($name,0,13).'...' : $name;?> 
                    ...
                    <ul class="dropdown-menu" >
						...
						<div>
						<?php if(!empty(Yii::$app->Permission->Profileimage($logged_id))){?>
                        <img src="<?php echo Yii::$app->getUrlManager()->getBaseUrl(); ?>/Images/profile_image/<?php echo Yii::$app->Permission->Profileimage($logged_id);?>" class="user-image" alt="User Image" class="img-circle" style="    height: 80px;width: 80px;float: initial;"/>
						<?php } ?>
