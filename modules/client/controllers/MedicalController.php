	...
	public function actionIndex() {
		if (\Yii::$app->SessionCheck->isclientLogged () == true) { // checking logged session
			...
			if (isset ( $_GET ['c_id'] ) && $_GET ['c_id'] != '') {
				...
				$company_detals=TblAcaCompanies::find()
				->select('company_client_number,company_name')
	...
	public function actionMedicaldata() {
		if (\Yii::$app->SessionCheck->isclientLogged () == true) { // checking logged session
			...
			if (! empty ( $_POST ['c_id'] )) {
				...
				$objMedicalData = new TblAcaMedicalData();
				...
				foreach ( $employees as $employee ) {
					...
					$data = array ();
					$data = $objMedicalData->medicalemploymentdata ( $employee ['employee_id'] );
	...
	public static function actionNewemployee() {
		if (\Yii::$app->SessionCheck->isclientLogged () == true) { // checking logged session
			...
			//	$record_exist = TblAcaMedicalData::findBySql ( $sql )->one ();
				...
				$record_exist=TblAcaMedicalData::find()
				->select('employee_id')
				->where(['ssn' => $ssn, 'company_id'=>$company_id])
				->one();
	...
	public function actionUpdateemployee() {
		if (\Yii::$app->SessionCheck->isclientLogged () == true) { // checking logged session
			...
			if (! empty ( $_POST ['newdata'] )) {
				...
				$ssn_exist=TblAcaMedicalData::find()
				->select('first_name')
				->where('ssn = :ssn AND company_id = :company_id AND employee_id !=:employee_id', ['ssn'=>$ssn, 'company_id'=>$company_id, 'employee_id'=>$model->employee_id])
				->one();
	...
	public function actionDeletepayrollemployee() {
		if (\Yii::$app->SessionCheck->isclientLogged () == true) { // checking logged session
			...
			if (! empty ( $_POST ['record'] )) {
				...
				$employment_model = TblAcaMedicalEmploymentPeriod::deleteAll ( [ 
						'employee_id' => $data ['employee_id'] 
				] );
	...
	public function actionAddemploymentperiod() {
		if (\Yii::$app->SessionCheck->isclientLogged () == true) { // checking logged session
			...
			if (! empty ( $_POST ['employee_details'] )) {
				...
				// initialising the model
				$model = new TblAcaMedicalEmploymentPeriod ();
	...
	public function actionUpdateemploymentperiod() {
		if (\Yii::$app->SessionCheck->isclientLogged () == true) { // checking logged session
			...
			if (! empty ( $_POST ['oldrecord'] )) {
				...
				// initialising the model
				$model = TblAcaMedicalEmploymentPeriod::findOne ( [ 
	...
	public function actionUploademployees() {
		...
		if (\Yii::$app->SessionCheck->isclientLogged () == true) { // checking logged session
			...
			if (isset ( $_GET ['c_id'] ) && $_GET ['c_id'] != '') {
				...
				$model_employment_period = new TblAcaMedicalEmploymentPeriod ();
							...
							try {
								do {
									...
									//	echo "<pre/>"; print_r($line);
										...
										if (!(empty($line [0]) || empty($line [2]) || empty($line [4]))) {
											$first_name='';
											$last_name='';
											$ssn='';
											$record_exist=array();
											...
											if(strlen($ssn)!=9){
												...
												$record_exist=TblAcaMedicalData::find()
												->select('employee_id')
												->where('ssn = :ssn AND company_id = :company_id', ['ssn'=>$ssn, 'company_id'=>$company_id])
												->one();
											...
											//	$resp = TblAcaLookupOptions::findBySql ( $sql )->one ();
												//echo $suffix;
												$resp=TblAcaLookupOptions::find()
												->select('lookup_id')
												->where('LCASE(lookup_value) LIKE :suffix AND code_id = :code_id', ['suffix'=>'%'.$suffix, 'code_id'=>7])
											...
											if ($model_employee->validate () && $model_employee->save ()) {
												...
												for($i = 0, $j = 12; $i < 12; $i ++, $j += 5) {
													...
													//	$sql="SELECT plan_class_id FROM tbl_aca_plan_coverage_type WHERE LCASE(plan_class_number) LIKE '%".$plan."%' AND company_id =";
													//	$resp=TblAcaPlanCoverageType::findBySql($sql)->one();
														
														$resp=TblAcaPlanCoverageType::find()
														->select('plan_class_id')
														->where('LCASE(plan_class_number) LIKE :plan AND company_id = :company_id', ['plan'=>'%'.$plan, 'company_id'=>$company_id])
														->one();
