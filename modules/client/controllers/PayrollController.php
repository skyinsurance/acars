	...
	public function actionPayrolldata() {
		if (\Yii::$app->SessionCheck->isclientLogged () == true) { // checking logged session
			...
			if (! empty ( $_POST ['c_id'] )) {
				...
				$objPayarollData = new TblAcaPayrollData ();
				
				// calling the model function to collect all the employess specific to company id
				$employees = $objPayarollData->payrolldata ( $company_id );
				...
				foreach ( $employees as $employee ) {
					...
					$data = array ();
					$data = $objPayarollData->employmentdata ( $employee ['employee_id'] );
				...
				$encoded_suffix = Json::encode ( $objPayarollData->Suffixes () );
	...
	public static function actionNewemployee() {
		if (\Yii::$app->SessionCheck->isclientLogged () == true) { // checking logged session
			...
			if (! empty ( $_POST ['newdata'] )) {
				...
				$record_exist=TblAcaPayrollData::find()
				->select('employee_id')
		...
		if (\Yii::$app->SessionCheck->isclientLogged () == true) { // checking logged session
			...
			if (! empty ( $_POST ['newdata'] )) {
				...
				if (! empty ( $newdata ['ssn'] )) {
					...
					$ssn_exist=TblAcaPayrollData::find()
					->select('first_name')
									...
									if (! empty ( $line )) {
										...
										if (! (empty ( $line [0] ) || empty ( $line [2] ) || empty ( $line [4] ))) {
											...
											if (strlen ( $ssn ) != 9) {
												...
												// $sql = "SELECT employee_id FROM tbl_aca_payroll_data WHERE LCASE(first_name) LIKE '%" . strtolower($first_name) . "%' AND LCASE(last_name) LIKE '%" . strtolower($last_name) . "%' AND company_id='".$company_id."'";
											//	$sql = "SELECT employee_id FROM tbl_aca_payroll_data WHERE ssn= '" . $ssn . "' AND company_id='" . $company_id . "'";
