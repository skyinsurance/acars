	...
	public function actionUpdateemployee() {
		if (\Yii::$app->SessionCheck->isclientLogged () == true) { // checking logged session
			...
			if (! empty ( $_POST ['newdata'] )) {
				...
				//print_r($model); die();
					$ssn=$newdata ['ssn'];
				$sql="SELECT first_name FROM tbl_aca_medical_data WHERE ssn='".$ssn."' AND company_id='".$company_id."' AND employee_id!='".$model->employee_id."'";
				$ssn_exist=TblAcaMedicalData::findBySql($sql)->one();
				// print_r($sql); die();
